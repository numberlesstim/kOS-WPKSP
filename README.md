...
`HUD` `F2`|Boolean|Get/Set|Is the HUD visible (F2)
`ShowHUD`|-|void Function|Show the HUD
`HideHUD`|-|void Function|Hide the HUD
`FlightResults` `FlightResult` `F3`|Boolean|Get/Set|Is the FlightResults GUI visible (F3)
`ToggleHUD`|-|void Function|Toggle the HUD
`ToggleNavBall`|-|void Function|Toggle the NavBall
`NavBall`|Boolean|Get/Set|Is the NavBall visible
`Altimeter` `AltitudeMode`|String|Get/Set|What mode is the altimeter displaying:<br><br>*"AGL"*, *"ASL"* or *"DEFAULT"*
`CrewPortraitsRight`<br><br>`CrewPortraitsLeft`|-|void Function|Click the 'RIGHT'/'LEFT' button on the Crew Portrait Gallery (bottom right corner)
`ExpandCrewPortraits`<br><br>`ShrinkCrewPortraits`|-|void Function|Click the 'ADD SLOT'/'REMOVE SLOT' button on the Crew Portrait Gallery (bottom right corner)
`SpawnRandomComet`|-|void Function|Make the game create and spawn a random comet
`DVInfo` `DeltaVInfo` `StageInfo`|Boolean|Get/Set|Is the staging stack displaying the expanded information panels
`Stages` `Staging` `StagingStack`|Boolean|Get/Set|Is the staging stack visible
`StageScroll` `StagesScroll` `StagingScroll` `StagingStackScroll`|Scalar|Get/Set|The vertical scroll position of the staging stack (0 = default)
